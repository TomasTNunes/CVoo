...
% LOS - characteristics
global actualpath Rmin distmin points Delta ref_path LOS;
actualpath = 0;
Rmin = 3500; % m
distmin = 1000; % m 1200
Delta = 800; % m
...
points=         [0         1.0000
                 ...
                 0.0000   -1.0000
                 0         1.0000] .* Rmin; % m
ref_path = diff(points);

% Run simulink
T_final = 1075; %s 1075
bb_stept = 10; %s
bb_ref = 0; %deg
out = sim('P5_F_Delta_sim',T_final);
...
% Plots path & path_ref % LOS points
figure()
plot(points(:,1),points(:,2),'*b')
hold on
plot(out.pos.data(:,1),out.pos.data(:,2),'-r','Linewidth',1.2)
plot(path(1,:),path(2,:),'--b','Linewidth',1.2)
plot(LOS(:,1),LOS(:,2),'*k','Linewidth',1.2)
xlabel('East [m]')
ylabel('North [m]')
legend('path_{ref} points','path','path_{ref}','LOS','Location','SouthEast')