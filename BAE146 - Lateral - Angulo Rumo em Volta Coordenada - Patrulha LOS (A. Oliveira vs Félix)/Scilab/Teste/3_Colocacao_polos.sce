...
pol_desejados = [-1.9291342;-0.0109763;-0.5079619+0.1093684*%i;-0.5079619-0.1093684*%i;-0.4607805]  //[-0.781+0.851*%i; -0.781-0.851*%i; -4.6; -1; -1.5]; //baseado no lgr, mas com mais margem
//[-0.5868666+0.7237439*%i; -0.5868666-0.7237439*%i; -0.7157403; -0.0135373;-0.1] // - polos LGR para referencia

//Cálculo da matriz ganho
K = ppol(A,B,pol_desejados)

//Definição do espaço de estados e calculo polos (check)
esp_est = syslin('c', A - B*K,B,C,D,x0); 
pol = spec(esp_est(2))
[wn, zz] = damp(pol)

//Calculo da matriz ganho de controlo
K_dc = D - (C/(A - B*K)*B);
K_dc = K_dc([1 5],:); //seleção do ganho do bb e psi ambos para da e dr
K_dc = inv(K_dc); //inversão da matriz 2x2 de ganho
...
if 1.2 < Tp then
    disp("Tp Rolamento maior que 1.2");
    disp(Tp);
end

//modo rolamento holandes
zz_rh = zz(3,1) // maior do que 0.19 (imposto pelo projeto maior que 0.6)
if zz_rh < 0.6 then
